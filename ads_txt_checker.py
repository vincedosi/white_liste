"""
MLI — Module ads.txt
Vérifie la présence du fichier ads.txt et parse les SSPs autorisés.

Le fichier ads.txt (Authorized Digital Sellers) est un standard IAB
qui liste les vendeurs autorisés à revendre l'inventaire publicitaire.
Un site sans ads.txt = risque de fraude publicitaire.
"""
...
class AdsTxtResult:
    ...
    raw_lines: int = 0
    seller_count: int = 0
    sellers: list[dict] = field(default_factory=list)  # top SSPs
    is_direct: bool = False  # a au moins un DIRECT
    is_reseller: bool = False  # a au moins un RESELLER
    top_ssps: list[str] = field(default_factory=list)  # noms uniques des SSPs
    error: str | None = None
...
# SSPs connus et leurs noms lisibles
SSP_NAMES = {
...
def parse_ads_txt_line(line: str) -> dict | None:
    ...
    domain, publisher_id, relationship, [cert_authority]

    Exemple: google.com, pub-1234567890, DIRECT, f08c47fec0942fa0
    """
    line = line.strip()
    ...
    # Ignorer les variables (CONTACT=, SUBDOMAIN=, etc.)
...
async def check_ads_txt(
    ...
    """Vérifie et parse le ads.txt d'un domaine."""
    ...
    try:
        ...
        result.sellers = sellers[:50]  # Limiter pour pas exploser la mémoire
...
async def check_all_ads_txt(domains: list[str]) -> dict[str, AdsTxtResult]:
    """Vérifie ads.txt pour tous les domaines."""
